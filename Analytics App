...
# =========================
# Sheet-aware loader (robust for this workbook + older)
# =========================
def load_sheet_structured(raw: pd.DataFrame):
    ...
    Date row above header is optional (only if present).
    """
    def row_text(i):
...
# =========================
def build_dataset_from_sheet(file_bytes, sheet_name):
    ...
    # Batch alignment ONLY for specified sheets (in-memory)
...
def kpi_cards(df, ctx, fdf):
    ...
    metric_row([
        ...
        ("Retention Rate", f"{retention_f:.1f}%" if payment_col else "N/A", f"All: {retention_all:.1f}%" if payment_col else "Needs Payment"),
    ])
      ...
      <div style="display:flex; align-items:center; justify-content:space-between; gap:12px;">
        <div>
          ...
          <div style="margin-top:4px; color:#2e6b57; font-weight:600;">
            Batch-aligned parsing for selected sheets • Cleaner insights • Conversion + retention + event intelligence
          </div>
        </div>
...
# ignore first 2 sheets (same as your original)
...
if mode == "Single sheet":
    ...
    # Focus on your specified batch sheets first; allow additional sheets too
    default_multi = [s for s in sheets if s in BATCH_ALIGNED_SHEETS]
    selected_sheets = st.sidebar.multiselect(
        "Select batch sheets to compare",
        options=sheets,
        default=default_multi if default_multi else sheets[:4],
    )
...
# In compare mode, aggregate into one df
df = pd.concat(dfs, ignore_index=True) if len(dfs) > 1 else dfs[0]
# Use first context as "primary" for column naming in UI; we also show per-sheet diagnostics separately
...
event_cols = primary_ctx["event_cols"]
event_dates = primary_ctx["event_dates"]
meta = primary_ctx["meta"]


# =========================
# Sidebar filters (apply to aggregated df too)
# =========================
st.sidebar.markdown("## Filters")

# conversion options present
...
# sheet filter (super useful in compare mode)
sheet_filter = None
...
with tab_students:
    ...
    st.dataframe(
        fdf.sort_values("participation_count", ascending=False)[
            [name_col, "participation_count", "conversion_category", "lead_score"] + (["__sheet__"] if "__sheet__" in fdf.columns else [])
        ].head(250),
        use_container_width=True,
        height=420,
    )
    ...
    with cA:
        ...
        targets = (
            ...
            [[name_col, "participation_count", "lead_score", "conversion_category"] + (["__sheet__"] if "__sheet__" in fdf.columns else [])]
            .head(25)
        )
    ...
    with cB:
        ...
        risks = (
            ...
            [[name_col, "participation_count", "lead_score"] + ([payment_col] if payment_col else []) + (["__sheet__"] if "__sheet__" in fdf.columns else [])]
            .head(25)
        )
...
with tab_conversion:
    ...
    with c2:
        ...
        st.dataframe(will_df[[c for c in cols_base if c != payment_col] + ([payment_col] if payment_col else [])], use_container_width=True, height=260)
    with c3:
        st.markdown("### 🟠 Pending")
        st.dataframe(pending_df[[c for c in cols_base if c != payment_col] + ([payment_col] if payment_col else [])], use_container_width=True, height=260)

    c4, c5 = st.columns(2)
    with c4:
        st.markdown("### 🔵 Refunded")
        st.dataframe(refunded_df[[c for c in cols_base if c != payment_col] + ([payment_col] if payment_col else [])], use_container_width=True, height=240)
    with c5:
        st.markdown("### 🔴 Not Paid")
        st.dataframe(not_df[[c for c in cols_base if c != payment_col] + ([payment_col] if payment_col else [])], use_container_width=True, height=240)
    ...
    # ✅ RESTORED: Engagement before/after payment date
...
with tab_quality:
    ...
    # Per-sheet diagnostics in compare mode
    diag_rows = []
    for sname, ctx_i in contexts:
        diag_rows.append({
            ...
            "Detected events": len(ctx_i["event_cols"]),
            "Has payment col": bool(ctx_i["payment_col"]),
            "Has batch col": bool(ctx_i["batch_col"]),
        })
    ...
    st.write("")
    st.subheader("Event date coverage")
    if event_cols:
        cover = pd.DataFrame({
            "Event": event_cols,
